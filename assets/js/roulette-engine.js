...
/* --- Site integration: restore saved demo balance for the logged in user, if any --- */
(function(){
	try {
		var session = localStorage.getItem('demoCasinoSession');
		if(session){
			var users = JSON.parse(localStorage.getItem('demoCasinoUsers') || '{}');
			if(users[session] && typeof users[session].balance === 'number'){
				bankValue = users[session].balance;
			}
		}
	} catch(e){ /* localStorage unavailable, fall back to default demo balance */ }
})();

let container = document.createElement('div');
container.setAttribute('id', 'container');
let mountPoint = document.getElementById('gameRoot') || document.body;
...
/* --- Site integration: periodically persist the demo balance for the logged in user --- */
setInterval(function(){
	try {
		...
		if(session){
			var users = JSON.parse(localStorage.getItem('demoCasinoUsers') || '{}');
			if(users[session]){
				users[session].balance = bankValue;
				localStorage.setItem('demoCasinoUsers', JSON.stringify(users));
			}
		}
